...
class SimpleCaptcha {
    ...
    public function CreateImage() {
        ...
        if(!empty($_REQUEST["session_var"]))
          $this->session_var = $_REQUEST["session_var"];
        
        if(empty($_SESSION[$this->session_var]) || !empty($_REQUEST["force_new"]))
        {
          $_SESSION[$this->session_var] = $this->GetCaptchaText();
        }

        //$msg = "Text for img new: " . $_SESSION[$this->session_var] . " (" . date("d.m.Y H:i:s") . ", session: " . session_id() . ", user: " . val_or_empty($_SESSION["user_name"]) . ")";
        //trace_message_to_file($msg, "captcha.log");
        
        $fontcfg  = $this->fonts[array_rand($this->fonts)];
        $this->WriteText($_SESSION[$this->session_var], $fontcfg);
        ...
        if ($this->debug) {
            imagestring($this->im, 1, 1, $this->height-8,
                $_SESSION[$this->session_var] . " {$fontcfg['font']} ".round((microtime(true)-$ini)*1000)."ms",
                $this->GdFgColor
            );
        }
    ...
    public function GetCaptchaText() {
        ...
        if (rand(1, 20) == 2) {
            ...
            $text = $own_words[array_rand($own_words)];
        }
    ...
    function GetDictionaryCaptchaText($extended = false) {
        ...
        $line   = rand(1, (int)(filesize($wordsfile)/$length)-2);
    ...
    protected function WaveImage() {
        ...
        for ($i = 0; $i < ($this->width*$this->scale); $i++) {
            imagecopy($this->im, $this->im,
                $i-1, (int) (sin($k+$i/$xp) * ($this->scale*$this->Xamplitude)),
                $i, 0, 1, (int) ($this->height*$this->scale));
        }
        ...
        for ($i = 0; $i < ($this->height*$this->scale); $i++) {
            imagecopy($this->im, $this->im,
                (int) (sin($k+$i/$yp) * ($this->scale*$this->Yamplitude)), $i-1,
                0, $i, (int) ($this->width*$this->scale), 1);
        }
    }
