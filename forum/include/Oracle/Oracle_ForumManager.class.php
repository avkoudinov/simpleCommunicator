<?php
//-------------------------------------------------------------------
// class Oracle_ForumManager
//-------------------------------------------------------------------
class Oracle_ForumManager extends ForumManager
{
    //-----------------------------------------------------------------
    ...
    function get_query_user($prfx, $where, $master_admin_name)
    {
        ...
        if (!empty($master_admin_name)) {
            $master_appendix = "
                ...
                select id, user_name
                from
                (select 0 id, '$master_admin_name' user_name from dual) master_admin
                $where
            ";
        }
    ...
    {
        return "blocked desc, coalesce(block_expires, current_timestamp + interval '1' year), registration_date desc";
    }
    ...
    function get_query_user_list($prfx, $start_date, $where, $order_by, &$pagination_info)
    {
        ...
        return "select
           ...
           order by $order_by
           offset $begin rows fetch next $pagination_info[rows_per_page] rows only";
    } // get_query_user_list
    ...
    function get_query_forum_topics($prfx, $uid, $user_pinned_topic_appendix, $where, &$pagination_info)
    {
        ...
        return "
          ...
          (select
           ...
           offset $begin rows fetch next $limit rows only) {$prfx}_topic
    ...
    function get_query_paginated_found_posts($prfx, $current_uid, $where, &$pagination_info, $order_by)
    {
        if ($pagination_info["mode"] == "all") {
            ...
            $limit = "offset $begin rows fetch next $pagination_info[posts_per_page] rows only";
        }
    ...
    function get_query_topic_cnt_update($prfx, $fid)
    {
        return "update {$prfx}_forum_statistics
                set
                    topic_count = coalesce((select coalesce(t_total_cnt.cnt, 0) - coalesce(t_invisible_cnt.cnt, 0)
                        from
                          (select count(*) cnt from {$prfx}_topic where forum_id = $fid) t_total_cnt,
                          (select count(*) cnt from {$prfx}_topic where (deleted = 1 or publish_delay = 1) and forum_id = $fid) t_invisible_cnt
                        ), topic_count),
                    topic_count_total = coalesce((select coalesce(t_total_cnt.cnt, 0) - coalesce(t_delayed_cnt.cnt, 0)
                        from
                          (select count(*) cnt from {$prfx}_topic where forum_id = $fid) t_total_cnt,
                          (select count(*) cnt from {$prfx}_topic where publish_delay = 1 and forum_id = $fid) t_delayed_cnt
                        ), topic_count_total),
                    last_message_date = coalesce((select {$prfx}_topic_statistics.last_message_date
                        from {$prfx}_topic
                        inner join {$prfx}_topic_statistics on ({$prfx}_topic.id = {$prfx}_topic_statistics.topic_id)
                        where forum_id = $fid and deleted <> 1 and publish_delay <> 1
                        order by {$prfx}_topic_statistics.last_message_id desc
                        fetch first 1 rows only), last_message_date),
                    last_message_id = coalesce((select {$prfx}_topic_statistics.last_message_id
                        from {$prfx}_topic
                        inner join {$prfx}_topic_statistics on ({$prfx}_topic.id = {$prfx}_topic_statistics.topic_id)
                        where forum_id = $fid and deleted <> 1 and publish_delay <> 1
                        order by {$prfx}_topic_statistics.last_message_id desc
                        fetch first 1 rows only), last_message_id)
    ...
    function get_query_post_cnt_update($prfx, $tid)
    {
        return "update {$prfx}_topic_statistics
                set
                    post_count = coalesce((select count(*) from {$prfx}_post where topic_id = $tid), 0)
                                 - coalesce((select count(*) from {$prfx}_post where topic_id = $tid and deleted = 1), 0),
                    post_count_total = coalesce((select count(*) from {$prfx}_post where topic_id = $tid), 0),
                    last_message_date = coalesce((select creation_date from {$prfx}_post
                        where topic_id = $tid and deleted <> 1
                        order by id desc fetch first 1 rows only), last_message_date),
                    last_message_id = coalesce((select id from {$prfx}_post
                        where topic_id = $tid and deleted <> 1
                        order by id desc fetch first 1 rows only), last_message_id)
    ...
    function get_query_user_post_cnt_update($prfx, $uid)
    {
        return "update {$prfx}_user_statistics
                set
                    post_count = coalesce((select count(*) from {$prfx}_post
                        where topic_id not in (select id from {$prfx}_topic where is_private > 0)
                        and {$prfx}_post.user_id = $uid), 0),
                    topic_count = coalesce((select count(*) from {$prfx}_topic
                        where is_private < 1 and {$prfx}_topic.user_id = $uid), 0)
    ...
    function get_query_user_rate_cnt_update($prfx, $uid)
    {
        return "update {$prfx}_user_statistics
                set
                    like_count = coalesce((select count(*) from {$prfx}_post
                        inner join {$prfx}_post_rating on ({$prfx}_post.id = {$prfx}_post_rating.post_id and rating = 1 and rater_ignored <> 1)
                        where {$prfx}_post.user_id = $uid), 0),
                    dislike_count = coalesce((select count(*) from {$prfx}_post
                        inner join {$prfx}_post_rating on ({$prfx}_post.id = {$prfx}_post_rating.post_id and rating = -1 and rater_ignored <> 1)
                        where {$prfx}_post.user_id = $uid), 0)
                where {$prfx}_user_statistics.user_id = $uid";
    ...
    function get_query_post_rate_cnt_update($prfx, $pid)
    {
        return "update {$prfx}_post_statistics
                set
                   like_count = coalesce((select sum(case when rating = 1 then 1 else 0 end)
                       from {$prfx}_post_rating where post_id = $pid), 0),
                   dislike_count = coalesce((select sum(case when rating = -1 then 1 else 0 end)
                       from {$prfx}_post_rating where post_id = $pid), 0)
    ...
    function get_query_fill_search_posts($prfx, $session_id, $now, $search_hash, $topic_part_where, $post_part_where, $max_search_results, $order_by, &$hints)
    {
        return "insert into {$prfx}_found_post_cache (post_id, topic_id, session_id, dt, search_hash)
                              ...
                              $post_part_where
                              $order_by
                              fetch first $max_search_results rows only
                             ";
    ...
    function get_query_found_topics($prfx, $current_uid, $session_id, $hash, &$pagination_info)
    {
        ...
        return "select
           ...
           (select
            ...
            offset $begin rows fetch next $pagination_info[rows_per_page] rows only) topics
    ...
    function get_topic_search_clause(&$dbw, $prfx, $search_keys, $with_morphology)
    {
        ...
        if (!empty($search_keys)) {
            // поиск точной фразы, например "exact revolution"
            if (preg_match('/^"(.+)"$/iu', $search_keys, $matches)) {
                $search_clause = str_replace("'", "''", $dbw->escape(trim($matches[1])));
                $where_clause = "contains({$prfx}_topic.name, '\"$search_clause\"') > 0";
                return $where_clause;
            }
            ...
            if (count($skey_array) == 0) {
                ...
                foreach ($skey_array as $skey) {
                    ...
                    if ($with_morphology && $this->get_unicore_words($skey, $words)) {
                        ...
                        foreach ($words as $word) {
                            ...
                            // Oracle Text: % — wildcard суффикса
                            $word_parts[] = str_replace("'", "''", $dbw->escape($word)) . "%";
                        }
                        if (!empty($word_parts)) {
                            $parts[] = "(" . implode(" OR ", $word_parts) . ")";
                        }
                    } else {
                        if (utf8_strlen($skey) < $db_ft_min_word_len) {
                            continue;
                        }
                        $parts[] = str_replace("'", "''", $dbw->escape($skey)) . "%";
                    }
                }
                $search_clause = implode(" AND ", $parts);
            } else {
                $parts = array();
                $op = "AND";
                ...
                foreach ($skey_array as $skey) {
                    ...
                    if ($with_morphology && $this->get_unicore_words($skey, $words)) {
                        ...
                        if (!empty($word_parts)) {
                            $parts[] = array("expr" => "(" . implode(" OR ", $word_parts) . ")", "op" => $op);
                        }
                    } else {
                        if (utf8_strlen($skey) < $db_ft_min_word_len) {
                            continue;
                        }
                        $parts[] = array("expr" => str_replace("'", "''", $dbw->escape($skey)) . "%", "op" => $op);
                    }
                ...
                if (!empty($parts)) {
                    $search_clause = $parts[0]["expr"];
                    for ($i = 1; $i < count($parts); $i++) {
                        $search_clause .= " " . $parts[$i]["op"] . " " . $parts[$i]["expr"];
                    }
                }
            }
    ...
    {
        // Oracle Text: использует contains() вместо tsvector-оператора @@
        // Индекс должен быть создан: CREATE INDEX idx_post_search ON {prfx}_post(text_content) INDEXTYPE IS CTXSYS.CONTEXT
        $where_clause = "";
        ...
        if (!empty($search_keys)) {
            if (preg_match('/^"(.+)"$/iu', $search_keys, $matches)) {
                $search_clause = str_replace("'", "''", $dbw->escape(trim($matches[1])));
                $where_clause = "contains({$prfx}_post.searchable_content, '\"$search_clause\"') > 0";
                return $where_clause;
            }
    ...
    function get_query_guest_last_activity($dbw, $prfx, $guest)
    {
        ...
        return "select dt, ip from {$prfx}_topic_view_history $where order by dt desc fetch first 1 rows only";
    ...
    function get_query_load_attachments($prfx, $uid, $current_appendex)
    {
        return "select {$prfx}_attachment.id, {$prfx}_attachment.post_id,
                                  ...
                                  extract(year from last_usage.creation_date) year,
                                  extract(month from last_usage.creation_date) month
    ...
    {
        // Oracle: интервал в секундах через NUMTODSINTERVAL
        return "select extract(hour from dt + NUMTODSINTERVAL($hour_offset, 'SECOND')) hour, sum(hits_count) hits_count
                from {$prfx}_forum_hits
                $where
                group by extract(hour from dt + NUMTODSINTERVAL($hour_offset, 'SECOND'))
                order by extract(hour from dt + NUMTODSINTERVAL($hour_offset, 'SECOND'))";
    } // get_query_user_hour_hits

    //-----------------------------------------------------------------
    function get_query_user_hour_posts($prfx, $where, $hour_offset)
    {
        return "select extract(hour from creation_date + NUMTODSINTERVAL($hour_offset, 'SECOND')) hour, count(*) post_count
                from {$prfx}_post
                $where
                group by extract(hour from creation_date + NUMTODSINTERVAL($hour_offset, 'SECOND'))
                order by extract(hour from creation_date + NUMTODSINTERVAL($hour_offset, 'SECOND'))
               ";
        ...
        return "select ip_summary.ip, first_attack, last_attack, attacks_count, atype, hits, hit_limit, check_period
               from
                 (select ip,
                  ...
                  fetch first 200 rows only) ip_summary
                inner join
                 (select ip,
    ...
    function get_query_subscribed_topics(&$dbw, $prfx, $uid, $mindate)
    {
        ...
        return "select coalesce({$prfx}_user.user_name, {$prfx}_topic.author) author, count(*) cnt
                from {$prfx}_topic
                left join {$prfx}_private_topics on ({$prfx}_topic.id = {$prfx}_private_topics.topic_id and {$prfx}_topic.is_private = 2 and {$prfx}_private_topics.participant_id = $uid)
                inner join {$prfx}_forum on ({$prfx}_topic.forum_id = {$prfx}_forum.id)
                inner join {$prfx}_user_subscription
                ...
                $ignore_forum_where_appendix
                $ignore_topic_where_appendix
                group by coalesce({$prfx}_user.user_name, {$prfx}_topic.author)";
    ...
    function get_query_fill_digest_posts($dbw, $prfx, $session_id, $now, $search_hash, $uid, $rm, $fid, $private_fid)
    {
        ...
        return "insert into {$prfx}_found_post_cache (post_id, topic_id, session_id, dt, search_hash)
                              ...
                              (
                                  ...
                                  and {$prfx}_post.creation_date > coalesce({$prfx}_topic_read_markers.last_read_date, {$prfx}_forum_read_markers.first_read_date)
    ...
    {
        $query = "delete from tmp_id_collector";
    ...
    function get_reply_post_clause($dbw, $prfx, $parent_pid, $deep)
    {
        ...
        return " and {$prfx}_post.id in (select $parent_pid as id from {$prfx}_dual
                                         union all
                                         select reply_post_id as id
                                         from {$prfx}_post_hierarchy
                                         start with parent_post_id = $parent_pid
                                         connect by prior reply_post_id = parent_post_id
                                         and level <= $level)";
...
{
    if (!$srdbw->execute_query("delete from tmp_children")) {
    ...
    $author_condition = !empty($author_id)
        ? "{$prfx}_post.user_id = $author_id"
        : "{$prfx}_post.author = $author";
    ...
    $query = "insert into tmp_children (id)
              ...
              where $author_condition
              and parent_post_id in (select id from tmp_children)
    ...
    function gen_load_statistics()
    {
        ...
        if (!empty($_SESSION["load_hits"]))            { ksort($_SESSION["load_hits"]); }
        if (!empty($_SESSION["exec_hits"]))            { ksort($_SESSION["exec_hits"]); }
        if (!empty($_SESSION["exec_times"]))           { ksort($_SESSION["exec_times"]); }
        if (!empty($_SESSION["topic_rm_count"]))       { ksort($_SESSION["topic_rm_count"]); }
        if (!empty($_SESSION["forum_rm_count"]))       { ksort($_SESSION["forum_rm_count"]); }
        if (!empty($_SESSION["total_topic_rm_count"])) { ksort($_SESSION["total_topic_rm_count"]); }
        if (!empty($_SESSION["total_forum_rm_count"])) { ksort($_SESSION["total_forum_rm_count"]); }
