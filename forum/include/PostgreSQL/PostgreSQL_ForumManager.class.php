<?php
//-------------------------------------------------------------------
// class PostgreSQL_ForumManager
//-------------------------------------------------------------------
class PostgreSQL_ForumManager extends ForumManager
{
    //-----------------------------------------------------------------
    ...
    function get_query_user_list($prfx, $start_date, $where, $order_by, &$pagination_info)
    {
        ...
        return "select
           ...
           order by $order_by
           limit $pagination_info[rows_per_page] offset $begin";
    } // get_query_user_list
    ...
    function get_query_forum_topics($prfx, $uid, $user_pinned_topic_appendix, $where, &$pagination_info)
    {
        ...
        return "
          ...
          (select 
           ...
           limit $limit offset $begin) {$prfx}_topic
    ...
    function get_query_paginated_found_posts($prfx, $current_uid, $where, &$pagination_info, $order_by)
    {
        if ($pagination_info["mode"] == "all") {
            ...
            $limit = "limit $pagination_info[posts_per_page] offset $begin";
        }
    ...
    function get_query_topic_cnt_update($prfx, $fid)
    {
        return "update {$prfx}_forum_statistics
                set
                    topic_count = coalesce(t_total_cnt.cnt, 0) - coalesce(t_invisible_cnt.cnt, 0),
                    topic_count_total = coalesce(t_total_cnt.cnt, 0) - coalesce(t_delayed_cnt.cnt, 0),
                    last_message_date = coalesce(t_last.last_message_date, {$prfx}_forum_statistics.last_message_date),
                    last_message_id = coalesce(t_last.last_message_id, {$prfx}_forum_statistics.last_message_id)
                from
                  (select forum_id,
    ...
    function get_query_post_cnt_update($prfx, $tid)
    {
        return "update {$prfx}_topic_statistics
                set
                    post_count = coalesce(ptcnt.cnt, 0) - coalesce(pdcnt.cnt, 0),
                    post_count_total = coalesce(ptcnt.cnt, 0),
                    last_message_date = coalesce(plast.last_message_date, {$prfx}_topic_statistics.last_message_date),
                    last_message_id = coalesce(plast.last_message_id, {$prfx}_topic_statistics.last_message_id)
                from
    ...
    function get_query_user_post_cnt_update($prfx, $uid)
    {
        return "update {$prfx}_user_statistics
                set
                    post_count = coalesce(user_post_count.cnt, 0),
                    topic_count = coalesce(user_topic_count.cnt, 0)
                from
    ...
    function get_query_user_rate_cnt_update($prfx, $uid)
    {
        return "update {$prfx}_user_statistics
                set
                    like_count = coalesce(user_like_count.cnt, 0),
                    dislike_count = coalesce(user_dislike_count.cnt, 0)
                from
    ...
    function get_query_post_rate_cnt_update($prfx, $pid)
    {
        return "update {$prfx}_post_statistics
                set
                   like_count = coalesce(cnt.likes, 0),
                   dislike_count = coalesce(cnt.dislikes, 0)
                from
    ...
    function get_query_found_topics($prfx, $current_uid, $session_id, $hash, &$pagination_info)
    {
        ...
        return "select
           ...
           (select
            ...
            limit $pagination_info[rows_per_page] offset $begin) topics
    ...
    function get_topic_search_clause(&$dbw, $prfx, $search_keys, $with_morphology)
    {
        ...
        if (!empty($search_keys)) {
            ...
            if (preg_match('/^"(.+)"$/iu', $search_keys, $matches)) {
                ...
                $where_clause = "{$prfx}_topic.search_vector @@ phraseto_tsquery('simple', '$search_clause')";
                return $where_clause;
            }
            ...
            if (count($skey_array) == 0) {
                ...
                foreach ($skey_array as $skey) {
                    ...
                    if ($with_morphology && $this->get_unicore_words($skey, $words)) {
                        ...
                        if (!empty($word_parts)) {
                            $parts[] = array("expr" => "(" . implode(" | ", $word_parts) . ")", "op" => $op);
                        }
                    } else {
                        if (utf8_strlen($skey) < $db_ft_min_word_len) {
                            continue;
            ...
            if (!empty($search_clause)) {
                $where_clause = "{$prfx}_topic.search_vector @@ to_tsquery('simple', '$search_clause')";
            }
        } // search_keys
    ...
    function get_post_search_clause(&$dbw, $prfx, $search_keys, $with_morphology)
    {
        ...
        if (!empty($search_keys)) {
            ...
            if (preg_match('/^"(.+)"$/iu', $search_keys, $matches)) {
                $search_clause = str_replace("'", "''", $dbw->escape(trim($matches[1])));
                $where_clause = "{$prfx}_post.search_vector @@ phraseto_tsquery('simple', '$search_clause')";
                return $where_clause;
            }
            ...
            if (!empty($search_clause)) {
                $where_clause = "{$prfx}_post.search_vector @@ to_tsquery('simple', '$search_clause')";
            }
        } // search_keys
    ...
    {
        return "select extract(hour from dt + interval '$hour_offset seconds') hour, sum(hits_count) hits_count
                from {$prfx}_forum_hits
                $where
                group by extract(hour from dt + interval '$hour_offset seconds')
                order by extract(hour from dt + interval '$hour_offset seconds')";
    } // get_query_user_hour_hits
    
    //-----------------------------------------------------------------
    function get_query_user_hour_posts($prfx, $where, $hour_offset)
    {
        return "select extract(hour from creation_date + interval '$hour_offset seconds') hour, count(*) post_count
                from {$prfx}_post
                $where
                group by extract(hour from creation_date + interval '$hour_offset seconds')
                order by extract(hour from creation_date + interval '$hour_offset seconds')
               ";
    ...
    function create_tmp_id_collector_table($dbw, $prfx)
    {
        ...
        // Note: index inside CREATE TEMPORARY TABLE is not supported in PostgreSQL,
        // create it separately (IF NOT EXISTS requires PostgreSQL 9.5+)
        $query = "create index if not exists idx_tmp_id on tmp_id_collector(id)";
        if (!$dbw->execute_query($query)) {
            MessageHandler::setError(text("ErrQueryFailed"), $dbw->get_last_error() . "\n\n" . $dbw->get_last_query());
            return "";
        }
    ...
    function get_query_fill_digest_posts($dbw, $prfx, $session_id, $now, $search_hash, $uid, $rm, $fid, $private_fid)
    {
        ...
        return "insert into {$prfx}_found_post_cache (post_id, topic_id, session_id, dt, search_hash)
                              ...
                              from
                              (
                                  select {$prfx}_post.id, {$prfx}_post.topic_id, '$session_id' session_id, cast($now as timestamp) dt, '$search_hash' search_hash,
