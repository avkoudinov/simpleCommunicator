...
require_once APPLICATION_ROOT . "Parsedown/Parsedown.php";
require_once APPLICATION_ROOT . "Parsedown/ParsedownExtra.php";
...
//------------------------------------------------------------------------------
function bb_process_markdown_simple($bbcode, $action, $name, $default, $params, $content)
{
    if ($action == BBCODE_CHECK) {
        return true;
    }
    ...
    if (empty($author)) {
        ...
        $author = escape_html($author);
        return "{$nl}[{{ai}} - $author]{$nl2}";
    }
} // bb_process_markdown_simple
//------------------------------------------------------------------------------
function bb_process_ai_simple($bbcode, $action, $name, $default, $params, $content)
{
    if ($action == BBCODE_CHECK) {
        return true;
    }
...
//------------------------------------------------------------------------------
function bb_process_hat_simple($bbcode, $action, $name, $default, $params, $content)
{
    if ($action == BBCODE_CHECK) {
        return true;
    }
...
function bb_process_hat($bbcode, $action, $name, $default, $params, $content)
{
    ...
    return "<span class='hat_link' onclick='show_hat()'>$content</span>";
}
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
function bb_process_ai($bbcode, $action, $name, $default, $params, $content)
{
    if ($action == BBCODE_CHECK) {
        return true;
    }
    ...
    $author = trim($default);
    if (empty($author)) {
        $author2 = escape_html("{{ai}}");
    } else {
        $author2 = make_links($author);
        $author = escape_html($author);
    }    
    
    return "\n\n" . '<div class="ai_wrapper"><div class="ai_header">' . $author2 . '</div><div class="ai">' . $content . '</div></div>' . "\n\n";
} // bb_process_ai
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
function bb_process_markdown($bbcode, $action, $name, $default, $params, $content)
{
    if ($action == BBCODE_CHECK) {
        return true;
    }
    ...
    $author = trim($default);
    if (empty($author)) {
        $author2 = escape_html("{{markdown}}");
    } else {
        $ai_class = "ai_markdown";
    ...
    return "<div class='markdown_wrapper $ai_class'><div class='markdown_header'>" . $author2 . "</div><div class='markdown'>" . $parsedown->text($content) . "</div></div>";
} // bb_process_markdown
//------------------------------------------------------------------------------
...
function bb_process_ok($bbcode, $action, $name, $default, $params, $content)
{
    if ($action == BBCODE_CHECK) {
        return true;
    }
    ...
    if (preg_match('/https:\/\/ok\.ru\/video\/(\d+)/i', $content ?? "", $matches)) {
        $code = $matches[1];

        if (preg_match('/[?&]fromTime=(\d+)/i', $content, $matches)) {
            $time = $matches[1];
        }
    }
    ...
    return gen_ok_html($code, $bbcode_text, $time);
} // bb_process_ok
//------------------------------------------------------------------------------
function check_ok_url($url, &$content, $message_mode)
{
    $time = "";

    if (preg_match('/https:\/\/ok\.ru\/video\/(\d+)/i', $url ?? "", $matches)) {
        if ($message_mode != "message") {
            $content = "\n[{{video}}: OK]\n\n";
            return true;
        }

        if (preg_match('/[?&]fromTime=(\d+)/i', $url, $matches2)) {
            $time = $matches2[1];
        }

        $content = gen_ok_html($matches[1], $url, $time);
...
function bb_process_rutube($bbcode, $action, $name, $default, $params, $content)
{
    ...
    if (preg_match('/https:\\/\\/rutube\\.ru\\/video\\/([^\\/\\?]+).*[?&]t=(\\d+).*/i', $content ?? "", $matches)) {
        $code = $matches[1];
...
{
    if (preg_match('/https:\\/\\/rutube\\.ru\\/video\\/([^\\/\\?]+).*[?&]t=(\\d+).*/i', $url ?? "", $matches)) {
        if ($message_mode != "message") {
            $content = "\n[{{video}}: RuTube]\n\n";
            return true;
        }
...
function bb_process_vkvideo($bbcode, $action, $name, $default, $params, $content)
{
    ...
    if (preg_match('/https:\/\/(m\\.)*(vk\.com|vk\.ru|vkvideo\.ru)(.*\/)(video|clip)(.*?)(\\?.*)?$/i', $content ?? "", $matches)) {
        $code = $matches[5];
    } elseif (preg_match('/(video|clip)(.*)$/i', $content ?? "", $matches)) {
        $code = $matches[2];
    }
...
{
    if (preg_match('/https:\/\/(m\\.)*(vk\.com|vk\.ru|vkvideo\.ru)(.*\/)(video|clip)(.*?)(\\?.*)?$/i', $url ?? "", $matches)) {
        if ($message_mode != "message") {
            $content = "\n[{{video}}: VK]\n\n";
            return true;
        }

        $content = gen_vkvideo_html($matches[5], $url);
        return true;
    }
...
function check_special_url($url, &$content, $message_mode)
{
    ...
    if (check_ok_url($url, $content, $message_mode)) {
        return true;
    }
...
function remove_nested_ais_bb(&$input, &$output, $limit)
{
    ...
    if (!preg_match_all('/(\[ai[^\[\]]*\]|\[\/ai\])/i', $output, $matches, PREG_OFFSET_CAPTURE)) {
        return true;
    }
...
//------------------------------------------------------
function gen_ok_html($code, $bbcode, $time)
{
    $appendix = "";
    if (!empty($time)) {
        $appendix = "&fromTime=$time";
    }

    $html = "<div class='media_wrapper' data-bbcode='" . escape_html($bbcode) . "'>";
    $html .= "<div class='short_video'><a class='ok_short_container' href='#' onclick='return show_embedded_video(this)'>OK</a></div>";
    $html .= "<div class='ok detailed_video'>";
    $html .= "<iframe width='640' height='500' src='//ok.ru/videoembed/$code?nochat=1$appendix' frameborder='0' allow='autoplay' allowfullscreen></iframe>";
...
function gen_vkvideo_html($code, $bbcode)
{
    ...
    try {
        ...
        $tp = "video";
		if (($json["response"]["items"][0]["type"] ?? "") == "short_video") {
            $tp = "clip";
		}
        ...
        if ($json) {
            ...
            } elseif (!empty($json["response"]["items"][0]["description"])) {
                $title = $json["response"]["items"][0]["description"];
            } else {
                $has_error = true;
            }
            ...
            if (!empty($json["response"]["items"][0]["player"])) {
                ...
                if (empty($parts[1])) {
                    ...
                    $player = "https://vk.com/clip_ext.php?oid={$parts[0]}&id={$parts[1]}";
                }
            } else {
                $has_error = true;
            }
    ...
    if ($has_error) {
        $html = "<div class='media_wrapper' data-bbcode='" . escape_html($bbcode) . "'><div class='short_video'><a class='vkvideo_short_container' href='https://vk.com/{$tp}{$code}' target='blank' onclick='return show_embedded_video(this)'>" . escape_html($title) . "</a></div>";
        ...
        $html .= "</div>";
        $html .= "<a class='attachment_link' href='https://vk.com/{$tp}{$code}' target='_blank'>{{link}}</a>";
        $html .= "</div>";
        return $html;
    }
    ...
    */
    ...
    $bbcode->AddRule('ai',
        array(
            ...
            'after_tag' => 'a',
            'before_endtag' => 'a',
            'method' => 'bb_process_ai',
            'allow_in' => array('listitem', 'block', 'columns', 'inline', 'link')
        ));
    //----------------------------------------------
    $bbcode->AddRule('markdown',
        array(
            'mode' => BBCODE_MODE_CALLBACK,
            'content' => BBCODE_VERBATIM,
            'before_tag' => 'a',
...
function parse_bb_code_simple(&$text, $mode = "email")
{
    ...
    remove_nested_ais_bb($text, $text, 2);
    ...
    $bbcode->AddRule('markdown',
        array(
            ...
            'after_tag' => 'a',
            'before_endtag' => 'a',
            'method' => 'bb_process_markdown_simple',
            'allow_in' => array('block', 'columns', 'inline', 'link')
        ));
    //----------------------------------------------
