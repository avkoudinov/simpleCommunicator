...
<th class="date_col"><?php echo_html(text("SubscriptionDate")); ?></th>
<th class="date_col"><?php echo_html(text("LastMessage")); ?></th>
<th class="date_col"><?php echo_html(text("ViewDate")); ?></th>
<th><?php echo_html(text("LastMessages")); ?></th>
<th><?php echo_html(text("LastTopics")); ?></th>
</tr>

<?php if(empty($subscribed_authors)): ?>
  <tr>
  <td colspan="7" class="table_message"><?php echo_html(text("UsersNotFound")); ?></td>
  </tr>
<?php else: ?>
  ...
  <div class="smart_break">
    <span><?php echo_html(text("AllAuthors")); ?></span>
  </div>  
  </td>
...
<?php foreach($subscribed_authors as $user_data): 
  ...
  ?>
  <tr>
  <td class="checkbox_selector" data-aname="<?php echo_html($aname); ?>" data-author="<?php echo_html($user_data["user_name"]); ?>" onclick="toggle_selection(this, '<?php echo_html($user_data["user_name"]); ?>'); show_author_actions_menu('<?php echo_html($aname); ?>')"><div>&nbsp;</div></td>
  <td class="author_col">
  ...
  <div class="popup_author_actions_menu" id="popup_author_actions_menu_<?php echo_html($aname); ?>">
      ...
      <a href="subscription.php" onclick='return select_all()'><?php echo_html(text("SelectAll")); ?></a>
      <a href="subscription.php" onclick='return unselect_all()'><?php echo_html(text("ResetSelection")); ?></a>
      
      <a href="subscription.php" onclick='return confirm_action("<?php echo_js(text("MsgConfirmUnubscribeFromUsers"), true); ?>", { unsubscribe_from_authors: 1 })'><?php echo_html(text("UnsubscribeFromUser")); ?></a>
      
      <a href="subscription.php" onclick='return do_action({ mark_read_action: "mark_subscriptions_read" })'><?php echo_html(text("MarkRead")); ?></a>
  ...
  <td class="date_col"><?php echo_html($user_data["tm"]); ?></td>
  <td class="date_col"><?php echo_html($user_data["last_post_date"]); ?></td>
  <td class="date_col"><?php echo_html($user_data["last_view"]); ?></td>
  <td>
  <?php
  $display = "style='display:none'";
...
<?php if(!empty($subscribed_authors) && count($subscribed_authors) > 25): ?>
