...
function reset_gallery_filter_dialog() {
    var elm;

    elm = document.getElementById('gallery_filter_forum');
    if (elm) elm.value = "";

    elm = document.getElementById('gallery_filter_start_date');
    if (elm) elm.value = "";

    elm = document.getElementById('gallery_filter_end_date');
    if (elm) elm.value = "";
}

function reset_gallery_filter_area() {
    already_loaded_ids = "";

    document.querySelectorAll('.attachment_gallery_area .attachment_thumb_separator, .attachment_gallery_area .attachment_thumb')
        .forEach(el => el.remove());
}

var already_loaded_ids = "";
function load_next_gallery_attachments(add_favourite_text, remove_favourite_text) {
    ...
    if (!load_next_gallery_attachments_ajax) {
        ...
        load_next_gallery_attachments_ajax.beforestart = function () {
            break_check_new_messages();
        };
        load_next_gallery_attachments_ajax.aftercomplete = function (error) {
            ...
            try {
                ...
                if (response.success && response.attachments) {
                    for (var i = 0; i < response.attachments.length; i++) {
                        ...
                        thumb.setAttribute("data-id", response.attachments[i].id);
                        ...
                        if (response.attachments[i].favourite == 1) {
                            ...
                            Forum.addXEvent(a, 'click', function (event) {
                                ...
                                return do_action({
                                    topic_action: "remove_attachment_from_favourites",
                                    id: this.getAttribute("data-att-id")
                                });
                            });
                        } else {
                            ...
                            Forum.addXEvent(a, 'click', function (event) {
                                ...
                                return do_action({
                                    topic_action: "add_attachment_to_favourites",
                                    id: this.getAttribute("data-att-id")
                                });
                            });
                        }
    ...
    elm = document.getElementById('gallery_filter_forum');
    if (elm) load_next_gallery_attachments_ajax.setPOST('forum', elm.value);

    elm = document.getElementById('gallery_filter_start_date');
    if (elm) load_next_gallery_attachments_ajax.setPOST('start_date', elm.value);

    elm = document.getElementById('gallery_filter_end_date');
    if (elm) load_next_gallery_attachments_ajax.setPOST('end_date', elm.value);

    const loadedImages = document.querySelectorAll('.attachment_thumb');

    loadedImages.forEach(image => {
        if (image.getAttribute('data-id')) already_loaded_ids += image.getAttribute('data-id') + ",";
    });

    load_next_gallery_attachments_ajax.setPOST('already_loaded_ids', already_loaded_ids);
