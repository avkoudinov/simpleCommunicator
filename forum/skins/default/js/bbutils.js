...
function convert_quote_to_bbcode(quote, quote_level) {
    console.log("convert_quote_to_bbcode");
    console.log(quote.outerHTML);
...
//----------------------------------------------------------------------
function convert_ai_to_bbcode(ai, quote_level) {
    console.log("convert_ai_to_bbcode");

    var title = "";
    var titles = ai.getElementsByClassName('ai_header');
    if (titles.length > 0) {
        var as = titles[0].getElementsByTagName('A');
    ...
    var body = "";
    var bodies = ai.getElementsByClassName('ai');
    if (bodies.length > 0) {
        body = convert_nodes_to_bbcode(bodies[0], quote_level);
    }
...
//----------------------------------------------------------------------
function convert_markdown_block_to_bbcode(markdown) {
    console.log("convert_markdown_block_to_bbcode");
    console.log(markdown.outerHTML);

    var title = "";
    var titles = markdown.getElementsByClassName('markdown_header');
    if (titles.length > 0 && markdown.classList.contains('ai_markdown')) {
        var as = titles[0].getElementsByTagName('A');
    ...
    var body = "";
    var bodies = markdown.getElementsByClassName('markdown');
    if (bodies.length > 0) {
        body = convert_markdown_html_to_bbcode(bodies[0]);
    }

    if (body == "") return "";

    return "\n\n[markdown=" + title + "]\n" + body + "\n[/markdown]\n\n";
} // convert_markdown_block_to_bbcode
//----------------------------------------------------------------------
//----------------------------------------------------------------------
// Strips the cache-busting "?d=1786903159011" suffix the forum appends
// to image URLs, so we recover the original src used in the markdown source.
function md_clean_img_src(src) {
    if (!src) return "";
    return src.replace(/\?d=\d+$/, "");
} // md_clean_img_src
//----------------------------------------------------------------------
function md_image(img) {
    var src = md_clean_img_src(img.getAttribute('src'));
...
//----------------------------------------------------------------------
// If "a" wraps nothing but a single IMG (ignoring blank text nodes),
// returns that IMG node. Otherwise returns null.
function md_get_wrapped_img(a) {
    var img = null;
    ...
    for (var i = 0; i < a.childNodes.length; i++) {
        ...
        if (c.nodeName == 'IMG') {
            if (img != null) return null; // more than one node
            img = c;
            continue;
        }

        return null; // some other element in there, not a pure image link
    }

    return img;
} // md_get_wrapped_img
//----------------------------------------------------------------------
function md_inline_node(n) {
    switch (n.nodeName) {
        case '#text':
        ...
        case 'A':
            ...
            var img = md_get_wrapped_img(n);

            if (img != null) {
                var img_md = md_image(img);
                var img_src = md_clean_img_src(img.getAttribute('src'));

                // the forum auto-wraps standalone images in a lightbox link that
                // points back at the image itself -- that's not a real markdown
                // link, so we drop the wrapper and keep just the image.
                if (n.classList.contains('lightbox_image') && href == img_src) {
                    return img_md;
                }
...
//----------------------------------------------------------------------
// Renders the inline (text-level) content of a container node.
function md_inline(container) {
    var out = "";

    for (var i = 0; i < container.childNodes.length; i++) {
        out += md_inline_node(container.childNodes[i]);
    }

    return out;
} // md_inline
//----------------------------------------------------------------------
function md_repeat(str, count) {
    var out = "";
    for (var i = 0; i < count; i++) out += str;
    return out;
} // md_repeat
//----------------------------------------------------------------------
function md_list(list, list_type, indent) {
    var lines = [];
    var counter = 1;
    ...
    for (var i = 0; i < list.childNodes.length; i++) {
        ...
        var marker = (list_type == 'ol') ? (counter++ + ".") : "-";
        var own = "";
        var nested = "";
        ...
        for (var j = 0; j < li.childNodes.length; j++) {
            ...
            if (c.nodeName == 'UL' || c.nodeName == 'OL') {
                nested += "\n" + md_list(c, c.nodeName == 'UL' ? 'ul' : 'ol', indent + 2);
            } else if (c.nodeName == 'P') {
                // Parsedown Extra wraps "loose" list item content in <p>
                own += md_inline(c);
            } else {
                own += md_inline_node(c);
            }
        }

        lines.push(md_repeat(" ", indent) + marker + " " + own.trim() + nested);
    }

    return lines.join("\n");
} // md_list
//----------------------------------------------------------------------
function md_pad(text, width) {
    text = text || "";
    var pad = width - text.length;
    if (pad < 0) pad = 0;
    return text + md_repeat(" ", pad);
} // md_pad
//----------------------------------------------------------------------
function md_table(table) {
    ...
    if (thead) {
        ...
        for (var i = 0; i < ths.length; i++) header_cells.push(md_inline(ths[i]).trim());
    }
    ...
    if (tbody) {
        ...
        for (var r = 0; r < trs.length; r++) {
            ...
            var cells = [];
            for (var c = 0; c < tds.length; c++) cells.push(md_inline(tds[c]).trim());
            rows.push(cells);
        }
    }

    var col_count = header_cells.length;
    if (col_count == 0 && rows.length > 0) col_count = rows[0].length;
    if (col_count == 0) return "";

    var widths = [];
    for (var c = 0; c < col_count; c++) {
        var w = header_cells[c] ? header_cells[c].length : 0;
    ...
    var header_line = "|";
    for (var c = 0; c < col_count; c++) header_line += " " + md_pad(header_cells[c], widths[c]) + " |";
    lines.push(header_line);

    var sep_line = "|";
    for (var c = 0; c < col_count; c++) sep_line += md_repeat("-", widths[c] + 2) + "|";
    lines.push(sep_line);

    for (var r = 0; r < rows.length; r++) {
        var row_line = "|";
        for (var c = 0; c < col_count; c++) row_line += " " + md_pad(rows[r][c], widths[c]) + " |";
        lines.push(row_line);
    }
...
//----------------------------------------------------------------------
function md_blockquote(bq) {
    var inner = md_block(bq);
...
//----------------------------------------------------------------------
function md_code_block(wrapper) {
    var lang = wrapper.getAttribute('data-code');
    if (!lang || lang == 'plaintext' || lang == 'nohighlight') lang = '';

    var body = "";
    var codes = wrapper.getElementsByTagName('code');

    if (codes.length > 0) {
        var code_html = codes[0].innerHTML;
        code_html = code_html.replace(/<span class="hljs-[^"]+">/g, "");
        code_html = code_html.replace(/<\/span>/g, "");

        body = Forum.decode_html(code_html);
    }

    return "```" + lang + "\n" + body + "\n```";
} // md_code_block
//----------------------------------------------------------------------
// Renders the block-level content of a container node.
function md_block(container) {
    var blocks = [];
    ...
    for (var i = 0; i < container.childNodes.length; i++) {
        ...
        switch (n.nodeName) {
            case '#text':
                if (n.textContent.trim() != '') blocks.push(n.textContent.trim());
                break;
            ...
            case 'UL':
                blocks.push(md_list(n, 'ul', 0));
                break;

            case 'OL':
                blocks.push(md_list(n, 'ol', 0));
                break;

            case 'TABLE':
                blocks.push(md_table(n));
                break;

            case 'BLOCKQUOTE':
                blocks.push(md_blockquote(n));
                break;

            case 'DIV':
                if (n.classList.contains('code_wrapper')) {
                    blocks.push(md_code_block(n));
                } else {
                    var inner = md_block(n);
                    if (inner != '') blocks.push(inner);
                }
                break;

            case 'PRE':
                var codes = n.getElementsByTagName('code');
                if (codes.length > 0) blocks.push(md_code_block(n));
                break;

            default:
                var text = md_inline(n);
...
//----------------------------------------------------------------------
function convert_markdown_html_to_bbcode(markdown) {
    console.log("convert_markdown_html_to_bbcode");
    console.log(markdown.outerHTML);

    var markdown_bb_code = md_block(markdown);

    // collapse accidental multiple blank lines and trim the ends
    markdown_bb_code = markdown_bb_code.replace(/\n{3,}/g, "\n\n");
    markdown_bb_code = markdown_bb_code.replace(/^\s+|\s+$/g, "");

    return markdown_bb_code;
} // convert_markdown_html_to_bbcode
//----------------------------------------------------------------------
...
function convert_nodes_to_bbcode(container, quote_level) {
    console.log("convert_nodes_to_bbcode");
    ...
    for (var i = 0; i < container.childNodes.length; i++) {
        ...
        switch (container.childNodes[i].nodeName) {
            ...
            case 'DIV':
                ...
                if (container.childNodes[i].classList.contains('message_container')) {
                    current_node = document.createTextNode('');
                    break;
                }
                ...
                if (container.childNodes[i].classList.contains('moderator_name')) {
                    text = convert_nodes_to_bbcode(container.childNodes[i], quote_level);
                    current_node = document.createTextNode("[b]" + text + "[/b]");
                    break;
                }
                if (container.childNodes[i].classList.contains('moderator_warning_text')) {
                    text = convert_nodes_to_bbcode(container.childNodes[i], quote_level);
                    current_node = document.createTextNode(text);
                    break;
                }
                ...
                if (container.childNodes[i].classList.contains('markdown_wrapper')) {
                    current_node = document.createTextNode(convert_markdown_block_to_bbcode(container.childNodes[i]));
                    break;
                }
                if (container.childNodes[i].classList.contains('markdown_header')) {
                    current_node = document.createTextNode('');
                    break;
                }
                if (container.childNodes[i].classList.contains('markdown')) {
                    text = convert_markdown_text_to_bbcode(container.childNodes[i]);
                    current_node = document.createTextNode("\n\n[markdown=]\n" + text + "\n[/markdown]\n\n");
                    break;
                }
                ...
                // we commented it to catch unhandled divs
                //current_node = document.createTextNode(text);
                break;
            ...
            case 'SPAN':
                if (container.childNodes[i].classList.contains('hat_link')) {
                    text = convert_nodes_to_bbcode(container.childNodes[i], quote_level);
                    current_node = document.createTextNode('[hat]' + text + '[/hat]');
                    break;
                }
