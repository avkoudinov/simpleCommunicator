...
?>
<span class="<?php echo($class); ?>" <?php echo($display); ?>><a href="<?php echo($digest_url); ?>" ><?php echo_html(text("Digest")); ?></a> <br></span>

<a href="<?php echo($base_url . $fpage_appendix); ?>" onclick='return confirm_action("<?php echo_js(text("MsgConfirmMarkRead"), true); ?>", { mark_read_action: "mark_favourites_read", uncritical: 1 })'><?php echo_html(text("MarkRead")); ?></a><br>
...
$not_preferred = "";
if(!empty($_SESSION["ignored_forums"][$fid]) && $fid != "-1" && $fid != "private") $not_preferred = "not_preferred";
?>

<?php echo_html(text("Forum")); ?>:
<a href="<?php echo($forum_url); ?>" class="<?php echo($not_preferred); ?>"><?php echo_html($tinfo["forum_name"]); ?></a>

<br><?php echo_html(text("Author")); ?>:

<?php if(empty($tinfo["user_id"])): ?>

<?php
$online_status = "";
if(empty($settings["hide_online_status"]) && empty($tinfo["author_ignored"]) && !empty($online_users["g_" . $tinfo["last_author"]]))
{
