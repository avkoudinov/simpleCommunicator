...
<a href="<?php echo($base_url_complete); ?>" onclick='return confirm_action("<?php echo_js(text("MsgConfirmMarkRead"), true); ?>", { mark_read_action: "mark_new_read", uncritical: 1 })'><?php echo_html(text("MarkRead")); ?></a><br>
...
<?php if(empty($settings["hide_online_status"]) && empty($_SESSION["skin_properties"][$skin]["no_online_users"]) && empty($is_private) && !empty($fid)): ?>
