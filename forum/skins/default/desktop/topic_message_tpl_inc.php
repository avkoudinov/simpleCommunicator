...
if(!empty($pinfo["editable"]) && empty($pinfo["is_system"]) && (($fmanager->is_admin() && empty($pinfo["topic_private"])) || $fmanager->is_forum_moderator($pinfo["forum_id"]) || $fmanager->is_topic_moderator($pinfo["topic_id"]))): ?>
<th id="post_head_<?php echo_html($pid); ?>" colspan="2" class="checkbox_post_selector <?php echo_html($current); ?> <?php echo_html($deleted); ?>" data-author="<?php echo_html($pinfo["topic_author"]); ?>" data-pid="<?php echo_html($pid); ?>" data-tid="<?php echo_html($pinfo["topic_id"]); ?>" data-subject="<?php echo_html(postprocess_message($pinfo["topic_name"])); ?>" data-profiled_topic="<?php echo_html($pinfo["profiled_topic"]); ?>" data-stringent_rules="<?php echo_html($pinfo["stringent_rules"]); ?>">
...
<div class="popup_moderator_menu" id="moderator_popup_menu_<?php echo_html($pid); ?>">
    ...
    <a href="<?php echo($message_url); ?>" class="moderator_link" onclick='return confirm_action("<?php echo_js(text("MsgConfirmToAdult"), true); ?>", { topic_action: "convert_to_adult", topic: "<?php echo_js($pinfo["topic_id"]); ?>", forum: "<?php echo_js($pinfo["forum_id"]); ?>" });'><?php echo_html(text("MakePostAdult")); ?></a>
    <a href="<?php echo($message_url); ?>" class="moderator_link" onclick='return confirm_action("<?php echo_js(text("MsgConfirmToNonAdult"), true); ?>", { topic_action: "convert_to_nonadult", topic: "<?php echo_js($pinfo["topic_id"]); ?>", forum: "<?php echo_js($pinfo["forum_id"]); ?>" });'><?php echo_html(text("MakePostNonAdult")); ?></a>

    <a href="<?php echo($message_url); ?>" class="moderator_link" onclick='return confirm_action_with_comment("<?php echo_js(text("MsgConfirmPostsDelete"), true); ?>", { topic_action: "delete_post", topic: "<?php echo_js($pinfo["topic_id"]); ?>", forum: "<?php echo_js($pinfo["forum_id"]); ?>" });'><?php echo_html(text("DeleteMessages")); ?></a>
    <a href="<?php echo($message_url); ?>" class="moderator_link" onclick='return confirm_action_with_comment("<?php echo_js(text("MsgConfirmPostsRestore"), true); ?>", { topic_action: "restore_post", topic: "<?php echo_js($pinfo["topic_id"]); ?>", forum: "<?php echo_js($pinfo["forum_id"]); ?>" });'><?php echo_html(text("RestoreMessages")); ?></a>
    
    <?php if(empty($pinfo["topic_private"])): ?>
    <a href="<?php echo($message_url); ?>" class="moderator_link" onclick="return select_target_topic_for_move('move_posts')"><?php echo_html(text("MovePosts")); ?></a>
    <?php endif; ?>
</div>

<div class="post_checkbox" onclick="toggle_selection(this, '<?php echo_html($pid); ?>'); show_moderator_popup_menu('<?php echo_html($pid); ?>')" data-pid="<?php echo_html($pid); ?>"></div>

<div class="<?php echo($citatable); ?> smart_break topic_title" style="float:left" data-pid="<?php echo_html($pid); ?>">
<?php echo($topic_name); ?><?php echo($pinned_appendix); ?>
</div>

<?php else: ?>
<th id="post_head_<?php echo_html($pid); ?>" colspan="2" class="<?php echo_html($pinned); ?> <?php echo_html($current); ?> <?php echo_html($deleted); ?>" data-author="<?php echo_html($pinfo["topic_author"]); ?>" data-pid="<?php echo_html($pid); ?>" data-tid="<?php echo_html($pinfo["topic_id"]); ?>" data-subject="<?php echo_html(postprocess_message($pinfo["topic_name"])); ?>" data-profiled_topic="<?php echo_html($pinfo["profiled_topic"]); ?>" data-stringent_rules="<?php echo_html($pinfo["stringent_rules"]); ?>">

<div class="<?php echo($citatable); ?> smart_break topic_title" style="float:left">
<?php echo($topic_name); ?><?php echo($pinned_appendix); ?>
</div>
<?php endif; ?>
...
<div class="post_id_info">
    ...
    <a href="<?php echo($message_url); ?>" id="favourite_post_link_<?php echo_html($pid); ?>" class="post_favourite_action post_in_favourites" onmouseout="this.blur()" onclick='return do_action({ topic_action: "remove_post_from_favourites", post: "<?php echo_js($pid); ?>" })' title="<?php echo_html(text("RemoveFromFavourites")); ?>">&nbsp;&nbsp;</a>&nbsp;
    <?php else: ?>
    <a href="<?php echo($message_url); ?>" id="favourite_post_link_<?php echo_html($pid); ?>" class="post_favourite_action post_not_in_favourites" onmouseout="this.blur()" onclick='return confirm_action("<?php echo_js(text("MsgConfrimAddPostToFavourites"), true); ?>", { topic_action: "add_post_to_favourites", post: "<?php echo_js($pid); ?>" })' title="<?php echo_html(text("AddToFavourites")); ?>">&nbsp;&nbsp;</a>&nbsp;
    <?php endif; ?>
...
<td rowspan="4" class="author_cell">
  ...
  <?php else: 
    ...
    $photo_appendix = "?rnd=" . rand(1000, 9999);
    if(!empty($user_data[$pinfo["user_id"]]["photo_ctime"])) $photo_appendix = "?ctime=" . $user_data[$pinfo["user_id"]]["photo_ctime"];
  ?>
  ...
  <?php if(!empty($user_data[$pinfo["user_id"]]["photo"])): ?>
  ...
  if(!empty($user_data[$pinfo["user_id"]]["notes"]))
  {
    ...
    $online_status .= "<div style='position: absolute;right:2px;top:2px;cursor:pointer' onclick='toggle_user_notes($pid)'><img src='{$view_path}images/cross.png' alt='" . escape_html(text('Close')) . "'></div>";
    $online_status .= $user_data[$pinfo["user_id"]]["notes"];
    $online_status .= "</div>";
  }
  ?>
  <div class="smart_break author_name"><a href="view_profile.php?uid=<?php echo_html($pinfo["user_id"]); ?>" ><?php echo_html($fmanager->get_display_name($pinfo["author"])); ?></a><?php echo($online_status); ?></div>
    ...
    <div class='from'><?php echo_html(text("Location")); ?>: <?php echo_html(val_or_empty($user_data[$pinfo["user_id"]]["location"])); ?><br></div>
    <?php endif; ?>

    <?php echo_html(text("Messages")); ?>: <span class="number"><?php echo_html(format_number(val_or_empty($user_data[$pinfo["user_id"]]["post_count"]))); ?></span><br>

    <?php if(!empty($settings["rates_active"])): ?>
    <div class='rating_info'><?php echo_html(text("Rating")); ?>:
    ...
    / <a href="search_topic.php?do_search=1&post_sort=desc&author_mode=author_disliked&author=<?php echo(xrawurlencode($pinfo["author"])); ?>" class="carma_minus" ><?php echo_html(val_or_empty($user_data[$pinfo["user_id"]]["carma_minus"])); ?></a></div>
    <?php endif; ?>
    <?php endif; ?>
    ...
    <?php echo(nl2br(escape_html(postprocess_message($user_data[$pinfo["user_id"]]["message"])))); ?>
    </div>
    <?php endif; ?>
...
<tr>
<td class="post_footer" style="border-right:0px"><?php echo_html(smart_date($pinfo["creation_date"])); ?></td>
<td class="post_footer" style="border-left:0px">

  <div class="user_post_actions">

  <?php if($may_write && $may_answer): ?>
  <span class="separator">|</span> <a href="<?php echo($message_url . "&do_answer=" . $pid . "&answer_author=" . xrawurlencode($pinfo["author"])); ?>" class="post_action_command answer" onclick='return answer_to_author("<?php echo_html($pid); ?>", "<?php echo_js($pinfo["author"], true); ?>", "<?php echo_js($pinfo["topic_id"], true); ?>", "<?php echo_js(postprocess_message($pinfo["topic_name"]), true); ?>", "<?php echo($pinfo["profiled_topic"]); ?>", <?php echo(!empty($pinfo["stringent_rules"]) ? 1 : 0); ?>);'><?php echo_html(text("Answer")); ?></a>
  <?php endif; ?>
  ...
  ?>
  <span class="separator">|</span> <a href="<?php echo($message_url . "&do_citate=" . $pid); ?>" class="post_action_command" onclick='return citate_post("<?php echo_html($pid); ?>", "<?php echo_js($pinfo["topic_id"], true); ?>", "<?php echo_js(postprocess_message($pinfo["topic_name"]), true); ?>", "<?php echo($pinfo["profiled_topic"]); ?>", <?php echo(!empty($pinfo["stringent_rules"]) ? 1 : 0); ?>);'><?php echo_html($citate_caption); ?></a>
  <?php endif; // may_answer ?>
  
  <?php if($may_write): ?>
  <span class="separator">|</span> <a href="<?php echo($message_url . "&do_write=" . $pid); ?>" class="post_action_command" onclick='return new_message("", "<?php echo_html($pid); ?>", "<?php echo_js($pinfo["topic_id"], true); ?>", "<?php echo_js(postprocess_message($pinfo["topic_name"]), true); ?>", "<?php echo($pinfo["profiled_topic"]); ?>", <?php echo(!empty($pinfo["stringent_rules"]) ? 1 : 0); ?>);'><?php echo_html(text("NewMessage")); ?></a> 
  <?php endif; // may write ?>
  ...
  ?>
  <span class="separator">|</span> <a  class="post_action_command" href="<?php echo($message_url); ?>" onclick='return show_post_comment("<?php echo_js(text("Complain"), true); ?>", "<?php echo_js($fmanager->get_display_name($pinfo["author"]), true); ?>", "<?php echo_js($pid); ?>", "complain")'><?php echo_html(text("Complain")); ?></a>
  <?php endif; ?>
  
  <?php if(!empty($pinfo["replies"])): ?>
  <span class="separator">|</span> <a  class="post_action_command" href="search.php?do_search=1&replies_to=<?php echo_html($pid); ?>&author_mode=wrote_post" target="_blank"><?php echo_html(text("Answers")); ?></a>
  <?php endif; ?>
  ...
  ?>
  | <a  class="post_action_command" href="topic.php?fid=<?php echo_html($pinfo["forum_id_for_url"]); ?>&tid=<?php echo_html($pinfo["topic_id"]); ?>&from_search=1<?php echo($search_keys_appendix); ?>&msg=<?php echo_html($pid); ?>" ><?php echo_html(text("GotoTopic")); ?></a> 
  <?php endif; ?>
 ...
 <div class="moderator_post_actions">
    ...
    <?php if(!empty($pinfo["editable"]) && empty($in_search) && !empty($pinfo["moderatable"]) && empty($pinfo["topic_private"])): ?>
      <span class='separator'>|</span> <a href="<?php echo($message_url); ?>" class="moderator_link" data-pid="<?php echo_html($pid); ?>" onclick="{ select_current(this); return select_target_topic_for_move('move_posts'); }"><?php echo_html(text("Move")); ?></a>
